...
q
        ...
        if self.framework.segmentation_class_value is None:
            ...
            # save warped_seg_moving 
            sizes = np.prod(seg1.shape.as_list()[1:])
            seg1 = tf.reshape(seg1, [-1, sizes])
            # warped_seg_moving = tf.reshape(warped_seg_moving, [-1, sizes])
            # warped_seg_moving = tf.reshape(warped_seg_moving, [128, 128, 128])
            # t = tf.constant(warped_seg_moving, tf.float32)
            # session = tf.Session()
            # array = session.run(t)
            # np.savez('./data_image/moved.npz',array)
            # img = sess.run(warped_seg_moving)
            jaccs = [jacc_score]
            dices = [dice_score]
        else:
